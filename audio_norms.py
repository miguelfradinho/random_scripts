...
def loudnorm_normalization(file_name):
    ...
    if not os.path.isfile(file_stats_file):
        ...
        if not os.path.isfile(stats_log):
            print(f"Starting loudnorm 1st pass for '{file_name}'... Writing to '{stats_log}'")
    ...
    with open(file_stats_file, "r") as j:
        ...
        print(f"Starting loudnorm 2nd pass for '{file_name}'... Writing to '{norm_out}'")
...
if __name__ == "__main__":
    if sys.platform == "win32":
        print("Windows is not supported because stuff with subprocess that i can't be asked to figure out, just run on linux (or if you want, check comments of code")
        ...
        parser.add_argument("-vd", "--detect", help="Applies 'volumedetect' filter", action="store_true")
        parser.add_argument("-l", "--loudnorm", help="Performs 2-pass loudnorm", action="store_true")
        parser.add_argument("-vi", "--volume", nargs="?", const=False, help="Increases volume level by provided dB value")
        ...
        if args.volume is not None:
            if args.volume:
                if args.detect:
                    print(f"WARNING: Both '-vd' and {args.volume} provided, using only value '{args.volume}' instead of reading from generated '-vd' file!")

                change_volume(file_name, args.volume)
            else:
                # args.volume == False , so only option was provided, no value
                if not args.detect:
                    sys.exit("No value provided! If you wish to adjust automatically to 0dB, please pass the '-vd' ('--volume') argument as well!")
                else:
                    print(f"WARNING: '-vd' passed, reading value from file at '{detected_file}'" )
                    line_value = None
                    ...
                    if abs(volume_change) <= 0.3:
                        print(f"Difference too small ({volume_change}) to change, exiting succesfully...")
                        exit()
                    else:
                        print(f"Value of {volume_change} read, increasing/decreasing accordingly")
